  ...
  return (
    <div className="container mx-auto px-6 py-12 animate-fadeIn pb-64">
      <div className="flex flex-col md:flex-row items-center justify-between mb-24 gap-12 text-center md:text-left">
        <div>
           <div className="inline-flex items-center space-x-4 text-honey mb-6">
              <i className="fas fa-play-circle text-5xl animate-pulse drop-shadow-[0_0_15px_#FFB800]"></i>
              <span className="text-[12px] font-black uppercase tracking-[0.5em]">Honey Premium Streaming</span>
           </div>
           <h1 className="text-6xl lg:text-9xl font-black honey-glow-text tracking-tighter uppercase leading-none">BILIMLAR<br/>MARKAZI</h1>
        </div>
        <button onClick={fetchVideos} className="btn-honey-neon px-12 py-7 text-[12px] tracking-widest font-black uppercase shadow-[0_0_40px_rgba(255,184,0,0.3)]">
           KUTUBXONANI YANGILASH <i className="fas fa-sync-alt ml-4"></i>
        </button>
      </div>

      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-64 space-y-12">
            <div className="w-24 h-24 border-8 border-honey border-t-transparent rounded-full animate-spin shadow-[0_0_50px_rgba(255,184,0,0.3)]"></div>
            <p className="text-honey font-black text-2xl uppercase tracking-[0.5em] animate-pulse">Ma'lumotlar saralanmoqda...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
          {videos.map((video) => {
            const interaction = userInteractions[video.id] || { liked: false, rating: 0, comments: [] };
            return (
              <div key={video.id} onClick={() => setSelectedVideo(video)} className="flex flex-col group space-y-10 animate-fadeIn cursor-pointer">
                <div className="aspect-video rounded-[4rem] border border-white/10 overflow-hidden relative shadow-2xl group-hover:border-honey/60 transition-all duration-700 bg-white/5">
                  <img src={video.img} alt={video.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 opacity-80 group-hover:opacity-100" />
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 backdrop-blur-md">
                     <div className="w-24 h-24 rounded-full bg-honey text-[#1A1100] flex items-center justify-center text-4xl shadow-[0_0_60px_rgba(255,184,0,0.5)] scale-75 group-hover:scale-100 transition-transform">
                        <i className="fas fa-play translate-x-1 drop-shadow-[0_2px_4px_rgba(0,0,0,0.2)]"></i>
                     </div>
                  </div>
                  <div className="absolute top-8 left-8">
                     <div className="bg-black/90 backdrop-blur-xl px-6 py-3 rounded-2xl text-[10px] font-black text-honey border border-honey/30 shadow-xl">
                        {video.duration}
                     </div>
                  </div>
                </div>
                <div className="px-8">
                   <h3 className="text-3xl font-black text-white group-hover:text-honey transition-colors line-clamp-2 leading-tight uppercase tracking-tighter">{video.title}</h3>
                   <div className="flex items-center justify-between mt-10 border-t border-white/5 pt-8">
                      <p className="text-gray-500 font-black uppercase tracking-widest text-[10px]">{video.channel}</p>
                      <div className="flex gap-10">
                         <span className="text-[12px] text-gray-400 font-black flex items-center gap-3 group-hover:text-white transition-colors">
                           <i className={`fas fa-heart text-xl ${interaction.liked ? 'text-honey drop-shadow-[0_0_15px_#FFB800]' : ''}`}></i>
                           {video.likes + (interaction.liked ? 1 : 0)}
                         </span>
                         <span className="text-[12px] text-gray-400 font-black flex items-center gap-3 group-hover:text-white transition-colors">
